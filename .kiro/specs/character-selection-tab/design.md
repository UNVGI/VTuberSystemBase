...
```
Packages/com.hidano.vtuber-system-base.character-selection-tab/
├── package.json
├── Runtime/
│   ├── Bootstrap/
...
- `com.hidano.vtuber-system-base.ui-toolkit-shell` 側の `UiToolkitShellSkinProfile.CharacterTabVisualTreeAsset` および `CharacterTabStyleSheets` に本 spec が提供する `CharacterTab.uxml` / `CharacterTab.uss` を参照させる運用（本 spec のパッケージ内に完結、`ui-toolkit-shell` 側のコード変更は不要）。
