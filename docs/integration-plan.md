...
| `com.hidano.runtime-display-selector` 0.1.1 | 物理ディスプレイ識別と切替 | runtime-display-selector-integration（後回し） |
...
| runtime-display-selector-integration | ❌ 未作成 | ❌ | ❌ | なし（後回し） |
...
### Wave 4: 後回し（並行開発と外部待ち）

| spec | 開始トリガ |
| --- | --- |
| `runtime-display-selector-integration` | RuntimeDisplaySelector の API が確定・リリースされる |
| 高機能カメラスイッチャー（PVW/PGM、トランジション） | 本フェーズの camera-switcher が動作確認できる |
| WebUI / LAN タブレット UI | LocalHost 通信のリモート許可と認証検討 |
...
[Wave 4]
    /kiro:spec-init "runtime-display-selector-integration: ..."
    （RDS 完成後）
```
...
| メイン出力カメラへの URP `targetDisplay` 振り分け | output-renderer-shell ↔ camera-switcher-output-adapter | Wave 3c 着手時 |
| RuntimeDisplaySelector との合流ポイント | output-renderer-shell の `IDisplayRoutingService` 差し替え | Wave 4 |
...
本統合計画の完了は次の 4 条件を満たすことで判定する。

1. Wave 3b〜3c の 6 spec すべてが `tasks` の必須項目を緑にしている。
2. `Assets/Scenes/MainDemo.unity` を PlayMode で起動するだけで、Display 1 に 3 タブ UI、Display 2 にキャラ + ステージ + ライト + カメラの映像が出る。
3. スタンドアロンビルドを実 2 画面で実行し、OBS で Display 2 を取り込んで配信に載せられる（手動受け入れ）。
4. `core-ipc-foundation` / `output-renderer-shell` / `ui-toolkit-shell` / 3 タブ / 3 アダプタの 9 パッケージが UPM パッケージとして他プロジェクトに `manifest.json` 経由で取り込めること。

> Wave 4（RuntimeDisplaySelector 連携）は本統合計画のスコープ外。完了後に独立 spec として進める。
...
依存最小・並行可・即着手可能な順としては **A → B（並行）→ Wave 3b** を推奨。
