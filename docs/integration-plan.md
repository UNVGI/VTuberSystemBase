...
| 担当 | パッケージ | 開始条件 | 完了条件 |
| --- | --- | --- | --- |
| Char | `com.hidano.vtuber-system-base.character-selection-tab` Contracts のみ | tasks 1.1〜1.3 | DTO/Topic 定数が UI 側・出力側からビルド参照可能 |
| Stage | `com.hidano.vtuber-system-base.stage-lighting-volume-tab` Contracts のみ | tasks 1.1〜1.3 | 同上 |
| Cam | `com.hidano.vtuber-system-base.camera-switcher-tab` Contracts のみ | tasks 1.1〜1.3 | 同上 |
...
> **実装状況 (Wave 3d 完了)**: `com.hidano.vtuber-system-base.integrated-demo` パッケージとして実装。`IntegratedDemoBootstrap` MonoBehaviour 1 つを空シーンに置けば全コンポーネントを `Awake` で結線する。`Samples~/IntegratedDemo/README.md` に手動シーン構築手順 (SkinProfile アセット作成、UXML 必須 region 名一覧、Standalone ビルド設定、L7 手動受け入れチェックリスト) を同梱。`.unity` シーンアセット自体は binary YAML のため Agent 生成不可で、README の手順通り手動構築が必要。
