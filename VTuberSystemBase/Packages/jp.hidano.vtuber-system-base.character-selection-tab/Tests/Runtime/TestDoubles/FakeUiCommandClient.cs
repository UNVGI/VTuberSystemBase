...
namespace VTuberSystemBase.CharacterSelectionTab.Tests.TestDoubles
{
    ...
    /// the test inject the response for the next <c>RequestAsync</c> call. Keep
    /// arguments boxed (object) so this single class handles every payload type
    /// without generic gymnastics.
    /// </summary>
    public sealed class FakeUiCommandClient : IUiCommandClient
    {
        ...
        public SendResult PublishState<TPayload>(string topic, TPayload payload)
        {
            ...
            return ForceFail ? SendResult.Fail(FailWith ?? new SendError(SendErrorCode.NotConnected)) : SendResult.Ok();
        }
